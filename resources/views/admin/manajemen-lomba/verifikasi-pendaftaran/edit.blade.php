<form action="" method="POST"enctype="multipart/form-data">
    ...
    <div class="modal-body">
        <div class="alert alert-danger">
                ...
                <strong class="alert-danger h4">Apakah anda yakin untuk merubah status data Pendaftaran lomba ini?</strong>
            </div>
        <!-- Informasi Lomba -->
        <div class="card mb-3">
        ...
        <div class="modal-footer">
            <button type="button" 
                onclick="submitTerima(this)" 
                class="btn btn-success"
                data-action="{{ route('verifikasi-pendaftaran.terima', $pendaftaran->pendaftaran_id)}}"> Terima
            </button>
            <button type="button" 
                onclick="submitTolak(this)" 
                class="btn btn-danger"
                data-action="{{ route('verifikasi-pendaftaran.tolak', $pendaftaran->pendaftaran_id)}}"> Tolak
            </button>
...
<script>
    function submitTerima(button) {
        const actionUrl = button.getAttribute('data-action');
        const token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');

        Swal.fire({
            title: 'Yakin ingin menerima pendaftaran ini?',
            icon: 'warning',
    ...
    function submitTolak(button) {
        if (document.querySelector('.modal.show')) {
            $('.modal').modal('hide'); // nutup modal lain 
        }
        const actionUrl = button.getAttribute('data-action');
        const token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
        ...
        Swal.fire({
            ...
            input: 'text',
            inputPlaceholder: 'Contoh: Tidak sesuai syarat lomba',
            showCancelButton: true,
