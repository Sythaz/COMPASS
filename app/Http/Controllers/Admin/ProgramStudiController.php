    ...
    public function list(Request $request)
    {
        ...
        return DataTables::of($dataProgramStudi)
            ...
            ->addColumn('aksi', function ($row) {
                ...
                $btn .= '<button style="white-space:nowrap" onclick="modalAction(\'' . url('/admin/master-data/program-studi/' . $row->prodi_id . '/delete_ajax') . '\')" class="btn btn-danger btn-sm">Nonaktif</button>';
                $btn .= '</div>';
                return $btn;
            })
            ->rawColumns(['aksi'])
